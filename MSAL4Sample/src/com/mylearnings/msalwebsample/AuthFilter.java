...
public class AuthFilter implements Filter
{
	...
	private List<String> excludedUrls = Arrays.asList("/", "/TestServlet/");
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException
	{
		...
		if (request instanceof HttpServletRequest)
		{
			...
			try
			{
				...
				String currentUri = "https://<domain-name>/aadCallbackServlet";
				...
				{
					// response should have authentication code, which will be
					// used to acquire access token
					authHelper.processAuthenticationCodeRedirect(httpRequest, currentUri, fullUrl);

					// remove query params so that containsAuthenticationCode
					// will not be true on future requests
				...
				{
					// not authenticated, redirecting to login.microsoft.com so
			...
			catch (MsalException authException)
			{
				...
				// we should invalidate AuthData stored in session and redirect
